...
tar_load(zip.pdf.all)
tar_load(zip.pdf.leitsatz)
tar_load(zip.txt)
tar_load(zip.csv.full)
tar_load(zip.csv.meta)
tar_load(zip.source)
...
\begin{verbatim}
 [gericht]_[spruchgruppe]_[leitsatz]_[datum]_[spruchkoerper_az]_
...
# Spruchgruppen am Bundespatentgericht
...
freqtable.spruchgruppe <- f.fast.freqtable(dt.bpatg.meta,
                                 varlist = "spruchgruppe",
                                 sumrow = TRUE,
                                 output.list = TRUE,
                                 output.kable = FALSE,
                                 output.csv = FALSE,
...
freqtable.spruchgruppe.nosum <- freqtable.spruchgruppe[-.N]

```



```{r, CE-BPatG_03_Barplot_Spruchgruppe, fig.height = 6, fig.width = 9}
ggplot(data = freqtable.spruchgruppe.nosum) +
    geom_bar(aes(x = reorder(spruchgruppe,
                             N),
                 y = N),
             stat = "identity",
             fill = "#005189",
    ...
    labs(
        title = paste(prefix.figuretitle,
                      "| Entscheidungen je Spruchgruppe"),
        caption = caption,
        x = "Spruchgruppe",
        y = "Entscheidungen"
    )+
...
```{r}
kable(freqtable.spruchgruppe,
      format = "latex",
      align = 'P{3cm}',
      booktabs = TRUE,
      longtable = TRUE,
      col.names = c("Spruchgruppe",
                    "Entscheidungen",
                    "% Gesamt",
                    "% Kumulativ"))  %>% kable_styling(latex_options = "repeat_header")
...
## Tabelle der Dateigrößen


```{r}
files.zip <- c(zip.pdf.all,
               zip.pdf.leitsatz,
               zip.txt,
               zip.csv.full,
               zip.csv.meta,
