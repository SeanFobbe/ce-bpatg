...
Mit der Option \enquote{format = "file"} werden für Input-Dateien Prüfsummen berechnet. Falls sich diese verändern werden alle von ihnen abhängigen Pipeline-Schritte als veraltet markiert und neu berechnet.
...
Mit diesen Targets werden CSV-Dateien eingelesen, die im file tracking zuvor definiert wurden.
...
list(
    ...
    tar_target(senatsgruppe_clean,
               f.clean_spruch_bpatg(dt.download$senatsgruppe))
    )
...
Durch diese Schritte werden die PDF-Dateien in TXT konvertiert und mitsamt den Variablen in ihren Dateinamen eingelesen. Beim Einlesen werden die in PDF-Dateien üblichen über Zeilen gebrochene Wörter wieder zusammengefügt.
