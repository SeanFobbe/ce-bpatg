...
#' Diese Funktion wertet die Entscheidungsdatenbank des Bundespatentgerichts aus und sammelt Links zu den Volltexten der Entscheidungen und verbindet sie mit den in der Datenbank angegebenen Metadaten.

#' @param files.html String. Pfade zu den HTML-Dateien der Datenbankseiten des BPatG.
...
f.parse_html_bpatg <- function(file){
    ...
    senatsgruppe <- rvest::html_nodes(html, "[class='ESpruchk']")
    senatsgruppe <- rvest::html_text(senatsgruppe, trim = TRUE)
    
    az <- rvest::html_nodes(html, "[class='EAz']")
